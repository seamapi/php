...
final class AccessCodesTest extends TestCase
{
  public function testAccessCodes(): void
  {
    ...
    try {
      ...
      $this->assertTrue(str_contains($exception->getMessage(), "Access Code Not Found"));
    }
    ...
    $formatted_starts_at = (new DateTime($time_bound_access_code->starts_at))->format("Y-m-d");
    $formatted_ends_at = (new DateTime($time_bound_access_code->ends_at))->format("Y-m-d");

    $seam->access_codes->update(access_code_id: $time_bound_access_code->access_code_id, type: "ongoing");
    $updated_access_code = $seam->access_codes->get(access_code_id: $time_bound_access_code->access_code_id);
